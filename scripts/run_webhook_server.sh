...
# 새 HEAD push 에 계속 추월되지 않게 한다. diff 는 excerpt 생성을 위한 hunk 범위 계산과
# GitHub 코멘트 anchor 에 모두 사용한다.
# 코드 기본값(review_service.DEFAULT_CURRENT_FILE_CONTEXT_MODE)은 CLI 호환을 위해 full 로 남겨둔다.
